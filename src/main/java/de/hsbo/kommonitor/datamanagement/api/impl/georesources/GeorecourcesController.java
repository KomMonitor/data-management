...
public class GeorecourcesController extends BasePathController implements GeoresourcesApi {
	...
	public ResponseEntity<ResponseEntity> deleteSingleGeoresourceFeatureById(
			...
			Principal principal) {
		logger.info("Received request to delete single georesource feature databse records for datasetId '{}' and featureId '{}'", georesourceId, featureId);
	...
	public ResponseEntity<ResponseEntity> deleteSingleGeoresourceFeatureRecordById(
			...
			Principal principal) {
		logger.info("Received request to delete single georesource feature databse record for datasetId '{}' and featureId '{}' and recordId '{}'", georesourceId, featureId, featureRecordId);
		...
		boolean isDeleted;
		try {
			isDeleted = georesourcesManager.deleteSingleGeoresourceFeatureRecordByRecordId(georesourceId, featureId, featureRecordId);
			lastModManager.updateLastDatabaseModification_georesources();
	...
	public ResponseEntity<ResponseEntity> updateGeoresourceFeatureRecordAsBody(
			...
			@ApiParam(value = "the unique database record identifier of the geo-resource dataset feature - multiple records may exist for the same real world object if they apply to different periods of validity", required = true) @PathVariable("featureRecordId") String featureRecordId,
			Principal principal) {
		logger.info("Received request to update single georesource feature database record for datasetId '{}' and featureId '{}' and recordId '{}'", georesourceId, featureId, featureRecordId);
		...
		try {
			georesourceId = georesourcesManager.updateFeatureRecordByRecordId(georesourceFeatureRecordData, georesourceId, featureId, featureRecordId);
			lastModManager.updateLastDatabaseModification_georesources();
		} catch (Exception e1) {
			return ApiUtils.createResponseEntityFromException(e1);
