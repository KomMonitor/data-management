...
//	EntityManager em;
	...
	private TopicsEntity tryPersistTopicWithoutSubtopics(TopicInputType topicData) throws Exception {
		...
		logger.info("Trying to persist topic with topicName '{}'", topicName);
		/*
		 * analyse input type
		 * 
		...
		if(topicsRepo.existsByTopicName(topicName)){
			logger.error("The topic with topicName '{}' already exists. Thus aborting add topic request.", topicName);
			throw new Exception("Topic already exists. Aborting add topic request.");
		}
		...
		topic.setTopicResource(topicData.getTopicResource());
