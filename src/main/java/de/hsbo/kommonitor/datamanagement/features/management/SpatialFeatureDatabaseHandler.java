...
public class SpatialFeatureDatabaseHandler {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(SpatialFeatureDatabaseHandler.class);
    ...
    public static String writeGeoJSONFeaturesToDatabase(ResourceTypeEnum resourceType, String geoJSONFeatures,
        ...
        org.geojson.FeatureCollection featureCollection_jackson = GEOJSON_MAPPER.readValue(geoJSONFeatures,
        ...
        DatabaseHelperUtil.updateResourceMetadataEntry(resourceType, featureSchema.getTypeName(),
    ...
    private static FeatureCollection toGeoToolsFeatureCollection(
        ...
        while (featureIterator.hasNext()) {
            ...
            SimpleFeature simpleFeature = featureJSON.readFeature(GEOJSON_MAPPER.writeValueAsString(jakcsonFeature));
            ...
            try {
                geotoolsFeatures.add(simpleFeature);
            } catch (DecodingException ex) {
                LOG.error("Decoding failed for feature {}", simpleFeature.getID());
                LOG.debug("Failed feature decoding attributes: {}", simpleFeature.getAttributes());
            }
        }

        return new ListFeatureCollection(featureSchema, geotoolsFeatures);

    }

    private static void persistSpatialResource(SimpleFeatureType featureSchema, FeatureCollection featureCollection,
                                               DataStore postGisStore) throws IOException {
    ...
    private static void addFeatureCollectionToTable(SimpleFeatureType featureSchema,
        ...
        try {
            ...
            LOG.error("Error while trying to add feature to store. Rolling back transaction.", eek);
...
//		tb.setDefaultGeometry(featureSchema.getGeometryDescriptor().getLocalName());
    ...
    public static AvailablePeriodsOfValidityType getAvailablePeriodsOfValidity(String dbTableName)
        ...
        AvailablePeriodsOfValidityType validityPeriods;
        ...
        try {
            ...
            handleClose(statement, jdbcConnection, rs);
            throw e;
        } finally {
            handleClose(statement, jdbcConnection, rs);
        }
    ...
    private static void handleClose(Statement stmt, Connection conn, ResultSet rs) {
        try {
            if (stmt != null) {
                stmt.close();
            }
    ...
    public static String getAllFeatures_withoutGeometry(String dbTableName) throws Exception {
        ...
        return getFeaturePropertiesArray(allGeoresourceFeatures);
    }

    private static String getFeaturePropertiesArray(String geoJsonFeatures)
            throws IOException {

        SpatialFeatureDatabaseHandler.instantiateFeatureJSON();

        ObjectMapper objectMapper = GEOJSON_MAPPER;
        ...
        List<Map<String, Object>> featureProperties = new ArrayList<>();
    ...
    public static String getValidFeatures_withoutGeometry(Date date, String dbTableName) throws Exception {
        ...
        return getFeaturePropertiesArray(validGeoresourceFeatures);
    }
...
//		Instant temporalInstant = new DefaultInstant(new DefaultPosition(date));
    ...
    private static void updateSpatialFeatureTable(String dbTableName, PeriodOfValidityType periodOfValidity,
                                                  String geoJsonString, Boolean isPartialUpdate) throws Exception {
        /*
        ...
        org.geojson.FeatureCollection inputFeatureCollection_jackson = GEOJSON_MAPPER.readValue(geoJsonString,
                org.geojson.FeatureCollection.class);
        ...
        List<SimpleFeature> newFeaturesToBeAdded = new ArrayList<>();
    ...
    private static void handleUpdateProcess(String dbTableName, Date startDate_new, Date endDate_new, FilterFactory ff,
                                            ...
                                            List<SimpleFeature> newFeaturesToBeAdded, Boolean isPartialUpdate) throws Exception {
    ...
    private static FeatureCollection retypeFeatureCollectionWithKomMonitorPropertiesIfNecessary(
        ...
        ArrayList<SimpleFeature> retypedFeaturesList = new ArrayList<>();
        ...
        return new ListFeatureCollection(targetSchema, retypedFeaturesList);
    }
    ...
    private static void compareSchemas(SimpleFeatureType inputFeatureSchema, SimpleFeatureType dbSchema,
        ...
        List<AttributeDescriptor> inputAttributeDescriptors = new ArrayList<>();
        List<AttributeDescriptor> dbAttributeDescriptors = new ArrayList<>();
        ...
        List<AttributeDescriptor> newProperties = new ArrayList<>();
    ...
    private static boolean isKomMonitorAttributeDescriptor(AttributeDescriptor attributeDescriptor) {
        ...
        return name.equals(KomMonitorFeaturePropertyConstants.GEOMETRY_COLUMN_NAME)
    ...
    private static void appendNewPropertyColumnsInDbTable(String dbTableName, List<AttributeDescriptor> newProperties)
        ...
        try {
            ...
            builder.append("ALTER TABLE \"").append(dbTableName).append("\" ");
            ...
            while (iterator.hasNext()) {
                ...
                builder.append("ADD COLUMN \"").append(property.getName()).append("\" varchar");
    ...
    private static void compareDbFeaturesToInputFeatures(String dbTableName, Date startDate_new, Date endDate_new,
        ...
        try {
            ...
            FeatureIterator<SimpleFeature> dbFeaturesIterator = dbFeatures.features();
    ...
    private static void compareInputFeatureToDbFeatures(Date startDate_new, Date endDate_new, FilterFactory ff,
        ...
        if (!correspondingDbFeatures.isEmpty()) {
    ...
    private static void compareFeatures(Date startDate_new, Date endDate_new, FilterFactory ff,
        ...
        correspondingDbFeatures.sort((feat1, feat2) -> {
    ...
    private static List<Feature> findCorrespondingDbFeaturesById(Feature inputFeature,
        ...
        List<Feature> identifiedDbFeatures = new ArrayList<>();
        ...
        if (identifiedDbFeatures.size() > 1) {
            identifiedDbFeatures.sort((o1, o2) -> {
    ...
    public static void deleteAllFeaturesFromFeatureTable(ResourceTypeEnum georesource, String dbTableName)
        ...
        try {
            ...
            builder.append("DELETE FROM \"").append(dbTableName).append("\"; ");

            if (!dbAttributeDescriptors.isEmpty()) {
                builder.append("ALTER TABLE \"").append(dbTableName).append("\" ");
                ...
                while (iterator.hasNext()) {
                    ...
                    // use dataType varchar, to import new columns as string
                    builder.append("DROP COLUMN \"").append(property.getName()).append("\" CASCADE ");
    ...
    public static SortedMap<String, String> guessSchemaFromFeatureValues(String allGeoresourceFeatures) {
        ...
        try {
            featureCollection_jackson = GEOJSON_MAPPER.readValue(allGeoresourceFeatures,
                    org.geojson.FeatureCollection.class);
        } catch (IOException e) {
            throw new RuntimeException("Failed to parse GeoJSON feature collection", e);
        }
        ...
        for (String key : properties.keySet()) {
            ...
            if (!Objects.equals(properties.get(key), "String")) {
...
//				ff.literal(featureRecordId), false);
    ...
    private static FeatureCollection fetchFeaturesForFeatureId(SimpleFeatureSource featureSource, String featureId)
        ...
        return featureSource.getFeatures(filter_featureId);
    }
...
//		updateSpatialFeatureTable(dbTableName, periodOfValidity, georesourceFeatureRecordData, isPartialUpdate);
        ...
        org.geojson.Feature inputFeature_jackson = GEOJSON_MAPPER.readValue(georesourceFeatureRecordData,
    ...
    private static Feature toGeoToolsFeature(org.geojson.Feature inputFeature_jackson,
                                             SimpleFeatureType inputFeatureSchema) throws IOException {
        FeatureJSON featureJSON = new FeatureJSON();

        return featureJSON.readFeature(GEOJSON_MAPPER.writeValueAsString(inputFeature_jackson));
    }
