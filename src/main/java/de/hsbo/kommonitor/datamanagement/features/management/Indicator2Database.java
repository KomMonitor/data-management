...
public class Indicator2Database {
	...
	public static boolean writeIndicatorsToDatabase(List<IndicatorPOSTInputTypeIndicatorValues> indicatorValues,
		...
		SimpleFeatureType featureType = createSimpleFeatureTypeForIndicators(postGisStore, ResourceTypeEnum.INDICATOR, availableDatesForIndicator);
        ...
        for (IndicatorPOSTInputTypeIndicatorValues indicatorEntry : indicatorValues) {
        	 ...
        	 * type has attributes
        	 * 1. spatialUnitId
        	 * 2. one attribute for each timeStamp
        	 */
...
//				String dateString = createDateStringForDbProperty(java.sql.Date.valueOf(timestamp));
		...
		DatabaseHelperUtil.updateResourceMetadataEntry(ResourceTypeEnum.INDICATOR, featureType.getTypeName().toString(), correspondingMetadataDatasetId);
	...
	private static SimpleFeatureType createSimpleFeatureTypeForIndicators(DataStore dataStore, ResourceTypeEnum resourceType, List<Date> availableDatesForIndicator) throws IOException {
