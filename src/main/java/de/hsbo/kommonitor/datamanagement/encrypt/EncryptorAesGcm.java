...
public class EncryptorAesGcm {
	...
	@Value("${encryption.symmetric.aes-gcm.tag.length_bit:128}")
	private int TAG_LENGTH_BIT; // must be one of {128, 120, 112, 104, 96}
	@Value("${encryption.symmetric.aes-gcm.iv.length_byte:12}")
	private int IV_LENGTH_BYTE;
	@Value("${encryption.symmetric.aes-gcm.salt.length_byte:16}")
